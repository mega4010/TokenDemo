...
import { MetamaskLogo, PhantomLogo } from "../ui/WalletLogos";
import { getPhantomEthereumProvider } from "./PhantomEthereumConnector.js";
...
const wallets = [
  ...
  { label: "Phantom", value: "phantom", icon: <PhantomLogo width={60} /> },
];
...
const NetworkWalletProviders = ({
  ...
  const { loginMetamask, loginInjected, loginPhantom } = useWalletConnector();
  ...
  const connectWallet = async (walletprovider) => {
    ...
    const isPhantom =
      normalized === "phantom_eth" || normalized === "phantom_bsc";
    if (typeof window !== "undefined") {
      if (isPhantom && !getPhantomEthereumProvider()) {
        console.error(
          "Phantom Ethereum not detected. Install Phantom from https://phantom.com and enable Ethereum.",
        );
        return;
      }
      if (!isPhantom && !window.ethereum) {
    ...
    const loginForPhantom = () => {
      if (loginPhantom) loginPhantom();
    };
    switch (normalized) {
      case "injected_eth":
